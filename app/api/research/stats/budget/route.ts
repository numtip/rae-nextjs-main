 ...
 *
 * Returns budget-specific analytics with multiple breakdowns:
 * by year, by funding type, by source, and by level.
 *
...
import type { BudgetStats, ApiError } from "@/lib/data/models";

export const dynamic = "force-static";

export async function GET(request: NextRequest): Promise<NextResponse<BudgetStats | ApiError>> {
  try {
    ...
    if (filters.budgetYears.length > 0) {
      ...
      filtered = {
        ...filtered,
        budgets: filtered.budgets.filter((b) => b.budgetYear !== null && yearSet.has(b.budgetYear)),
      };
    }
    ...
    if (filters.moneyNames.length > 0) {
      ...
      filtered = {
        ...filtered,
        budgets: filtered.budgets.filter((b) => b.moneyName !== null && moneySet.has(b.moneyName)),
      };
    }
    ...
    const stats = computeBudgetStats(filtered);
    ...
    console.error("GET /api/research/stats/budget failed:", error);
    return NextResponse.json(
      {
